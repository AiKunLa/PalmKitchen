...
11. 添加用户信息修改组件
    使用react-vant 中的Form 组件 来实现用户信息的修改
        个人资料编辑表单 ：包含用户名、性别、生日、职业、家乡和个人介绍等字段的完整编辑功能
    头像管理系统 
        点击头像区域弹出操作菜单
        支持通过 AI 生成头像：通过coze工作流来实现用户头像的生成
    表单数据管理
        表单提交按钮集成在顶部导航栏
        使用 Form.useForm 管理表单状态和验证
        从全局状态 `useAccountStore` 获取和更新用户信息
